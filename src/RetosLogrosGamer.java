...
public class RetosLogrosGamer {
    public static void main(String[] args) {
        ...
        int danoHecho = 2100;
        int danoRecibido = 451;
        int oro = 100;
        int KDA = (kills + asistencias) / max(1, muertes);

        System.out.println("=== RANGO DE JUGADOR ===");

        // Comprobar si tuvo RQny dar logro negativo
        boolean tuvoRQ = (desconexion && tiempoMin < 5);
        //2
        if (tuvoRQ) {
            System.out.println("\nLogro NEGATIVO: Rge Quit");
        } else if (KDA >= 5 && (tuvoRQ == false)) {
            // (tuvoRQ == false) es lo mismo que tuvo que !tuvoRQ
            if (KDA >= 5 && tuvoRQ) {
            ...
            if (kills >= 10) ;
            {
                System.out.println("Logro: CAzador Experto desbloqueado");
            }
            ...
            if (kills >= 15 && muertes <= 2) {
                System.out.println("Logro: Victoria Agresiva");
            }
            if (oro >= 2000) {
                System.out.println("Logro: Ahorrista de V-Bucks");
            }
            ...
            {
                System.out.println("Partida Individual AFK");
            }
            if (muertes == 0 && kills == 0 && asistencias == 0 && tiempoMin < 2) {
